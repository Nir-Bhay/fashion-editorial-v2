...
**Learning:** For icon-only interactive elements, adding a `title` attribute is necessary to provide native tooltips for sighted users. This must be paired with an `aria-label` for screen readers, and the decorative icons/notification dots inside should be hidden using `aria-hidden="true"`. Furthermore, when visual state changes (like a notification dot appearing), the `aria-label` must explicitly convey the state (e.g., changing "Cart" to "Cart with items").
**Action:** When adding icon-only buttons, always ensure they have both `title` and `aria-label`. Use `aria-hidden="true"` on the internal decorative elements. Update `aria-label` based on visual notification states to keep screen readers in sync.
