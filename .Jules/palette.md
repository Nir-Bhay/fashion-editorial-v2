...
## 2024-05-18 - Restoring Keyboard Focus on Stretched Links
**Learning:** When using the "stretched link" pattern via a pseudo-element (e.g., `after:absolute after:inset-0`), standard focus rings on the anchor element itself are often hidden or behave unexpectedly because the pseudo-element expands over the whole container.
**Action:** Apply focus utilities directly to the pseudo-element (e.g., `focus-visible:after:ring-2 focus-visible:after:ring-charcoal`) instead of the base element, ensuring the focus indicator correctly outlines the entire interactive area.
