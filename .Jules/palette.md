...
## 2026-05-16 - Native Tooltips and Dynamic ARIA Labels on Icon Buttons
**Learning:** Icon-only interactive elements benefit significantly from having both an `aria-label` (for screen readers) and a `title` attribute (as a native tooltip for sighted users). Furthermore, when purely decorative visual elements (like a notification dot) indicate state, they should be hidden from screen readers using `aria-hidden="true"`, while the parent element's `aria-label` should explicitly communicate that state (e.g., "Cart with items").
**Action:** Always pair `title` with `aria-label` on icon-only buttons unless custom tooltips are implemented. Update parent `aria-labels` to reflect state changes instead of relying on screen readers to parse decorative notification indicators.
