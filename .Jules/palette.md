...
## 2024-05-18 - Keyboard Focus on Pseudo-Elements
**Learning:** When using `::after` pseudo-elements to stretch links over entire cards (a common pattern for expanding click targets), setting `focus:outline-none` on the parent link destroys keyboard navigation accessibility because the focus ring is hidden. Furthermore, simply adding standard `focus-visible:ring` to the link doesn't always render correctly over the card bounds.
**Action:** Always apply the `focus-visible` utility classes directly to the pseudo-element itself (e.g., `focus-visible:after:ring-2 focus-visible:after:ring-offset-4`) when implementing stretched link patterns to guarantee proper visual indicators for keyboard users without disrupting the visual layout.
