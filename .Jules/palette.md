...
## 2024-05-24 - Transient Loading States & React Effect Dependency Management
**Learning:** When implementing multi-stage transient UI states (e.g., loading -> success -> reset) to provide accessible micro-UX feedback, setting side-effect states (like `aria-live` text) synchronously at the top level of a `useEffect` alongside asynchronous timeout state updates can trigger ESLint `react-hooks/set-state-in-effect` errors and cascading renders. Furthermore, combining sequential timeouts within a single `useEffect` causes premature cleanup if the first timeout updates a state variable tracked in the dependency array.
**Action:** When managing sequential transient states, use separate `useEffect` hooks for each timeout stage (e.g., one for simulating the loading delay, another for clearing the success state). Always set side-effect states simultaneously with the triggering state inside a `setTimeout` callback or event handler to prevent `set-state-in-effect` errors, and ensure timeouts are properly cleared if the component unmounts. Always complement visual state transitions with a visually hidden `aria-live="polite"` region.
