...
## 2024-06-08 - Async Loading State Accessibility
**Learning:** When implementing async loading states (like "Load More"), a visual spinner alone is insufficient for screen readers. Connecting the button's `aria-label` isn't always enough to communicate state changes reliably during the lifecycle of the request.
**Action:** Always pair visual loading indicators (like spinners and disabled buttons) with a visually hidden `aria-live="polite"` region. This region should explicitly announce the start (e.g., "Loading...") and completion (e.g., "Successfully loaded") of the asynchronous operation, ensuring screen reader users are fully informed of the interaction's progress. Use two separate `useEffect` hooks for the start and completion to avoid premature cleanup.
