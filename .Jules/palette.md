...
## 2026-05-15 - Icon-only Button Tooltips & Redundant SVGs
**Learning:** Icon-only buttons often need both `aria-label` (for screen readers) and `title` (for native tooltips for sighted users). Furthermore, `svg` elements or notification dot `span`s inside these buttons should have `aria-hidden="true"` to prevent screen readers from reading them out redundantly, since the button's `aria-label` already provides the necessary context.
**Action:** When creating or modifying icon-only buttons, always ensure they have both `aria-label` and `title`. Always add `aria-hidden="true"` to decorative elements (like SVGs or status dots) inside buttons that already have accessible names.
