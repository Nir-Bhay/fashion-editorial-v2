...
function App() {
  ...
  useEffect(() => {
    if (isLoading) {
      const timer = setTimeout(() => {
        setIsLoading(false);
        setLoadStatus('Successfully loaded more items');
      }, 1500);
      return () => clearTimeout(timer);
    }
  }, [isLoading]);

  // Handle success message cleanup timeout
  useEffect(() => {
    if (loadStatus === 'Successfully loaded more items') {
      const timer = setTimeout(() => {
        setLoadStatus('');
      }, 3000);
      return () => clearTimeout(timer);
  ...
  const handleLoadMore = () => {
    setIsLoading(true);
    setLoadStatus('Loading more items...');
  };
            ...
            <div className="col-span-12 lg:col-span-10">
              ...
              <div className="mt-24 flex flex-col items-center justify-center">
                 ...
                 <button
                   ...
                   disabled={isLoading}
                   className={`px-12 py-4 border border-charcoal text-sm uppercase tracking-widest transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-charcoal focus-visible:ring-offset-2 rounded-sm flex items-center justify-center space-x-2 ${
                     isLoading
                       ? 'bg-charcoal text-white/50 cursor-not-allowed'
