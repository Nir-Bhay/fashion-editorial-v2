  ...
  useEffect(() => {
    if (isLoadingMore) {
      const timer = setTimeout(() => {
        ...
        setLoadMessage('Successfully loaded more items');
              ...
              <div className="mt-24 flex justify-center flex-col items-center">
                 ...
                 <button
                    ...
                    className={`px-12 py-4 border border-charcoal text-sm uppercase tracking-widest transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-charcoal focus-visible:ring-offset-2 rounded-sm flex items-center justify-center min-w-[200px] ${
                      isLoadingMore
                        ? 'bg-charcoal/5 text-charcoal/50 cursor-not-allowed border-charcoal/20'
                        : 'hover:bg-charcoal hover:text-white'
                    }`}
                 >
                    {isLoadingMore ? (
                      <>
                        <svg className="animate-spin -ml-1 mr-3 h-4 w-4 text-charcoal/50" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
