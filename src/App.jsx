...
function App() {
  ...
  useEffect(() => {
    if (isLoadingMore) {
      const timer = setTimeout(() => {
        ...
        setAriaMessage('Successfully loaded 9 new items');
  ...
  useEffect(() => {
    if (loadMoreSuccess) {
      const timer = setTimeout(() => {
        ...
        setAriaMessage('');
      }, 3000); // Success message visibility duration
      return () => clearTimeout(timer);
    }
  }, [loadMoreSuccess]);
  ...
  const handleLoadMore = () => {
    if (!isLoadingMore && !loadMoreSuccess) {
      ...
      setAriaMessage('Loading more items...');
    }
  };
            ...
            <div className="col-span-12 lg:col-span-10">
              ...
              <div className="mt-24 flex flex-col items-center justify-center">
                 ...
                 <button
                    ...
                    disabled={isLoadingMore || loadMoreSuccess}
                    className={`px-12 py-4 border border-charcoal text-sm uppercase tracking-widest transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-charcoal focus-visible:ring-offset-2 rounded-sm flex items-center justify-center min-w-[200px] ${
                      isLoadingMore || loadMoreSuccess
                        ? 'opacity-80 cursor-not-allowed bg-charcoal/5 text-charcoal'
                        : 'hover:bg-charcoal hover:text-white'
                    }`}
                 >
                    {isLoadingMore ? (
                      <>
                        <svg className="animate-spin -ml-1 mr-3 h-4 w-4 text-charcoal" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      ...
                      <>
                        <svg className="mr-2 h-4 w-4 text-charcoal" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
