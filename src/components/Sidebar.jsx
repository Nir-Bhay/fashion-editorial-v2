      ...
      <div className="mb-10">
        ...
        <ul className="space-y-3">
          {categories.map((cat, idx) => (
            <li key={cat.name}>
              <button
                className={`w-full flex justify-between items-center group cursor-pointer py-1 text-left ${idx === 0 ? 'text-charcoal font-medium' : 'text-charcoal/60'}`}
              >
            ...
            <div>
               ...
               <div className="flex flex-wrap gap-2">
                  {colors.map((color) => (
                    <button
                      key={color.name}
                      className={`w-6 h-6 rounded-full ${color.class} hover:scale-110 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-charcoal focus-visible:ring-offset-2`}
